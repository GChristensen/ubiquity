...
function getFeeds(type) { return [
  for (feed of feedManager["get" + type + "Feeds"]())
  if ("commands" in feed) feed
] }
...
function actionLink(text, action) {
  return $("<span></span>").text(text).click(action).addClass("action")
}
    ...
    cell.append(actionLink(
      ...
      function unsubscribe() {
        ...
        cell.slideUp(function onUnsubscribe() {
          $(".id").filter((i, x) => x.name.startsWith(feedUrl))
            .closest("tr").remove();
...
function formatAuthors(authors)
  [for (a of [].concat(authors)) formatAuthor(a)].join(", ");
...
function buildTable() {
  ...
  function addFeedToTable(feed) {
    let cmdIds = Object.keys(feed.commands);
    let feedCell = $("<td></td>");
    let {length} = cmdIds;
  ...
  if (/^feed/.test(sortMode))
    ...
    sortCmdListBy([...commandSource.getAllCommands()],
...
function makeUnsubscribedFeedListElement(info) {
  var $li = $("<li></li>").append(
    ...
    "<ul>" +
    Utils.values(info.commands).map(
