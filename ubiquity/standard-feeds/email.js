...
function findGmailTab() {
  return Utils.tabs.search(/^https?:\/\/mail\.google\.com\/mail\/(?:[?#]|$)/)[0]
}
...
CmdUtils.CreateCommand({
  ...
  help: '\
    Currently only works with <a href="http://mail.google.com">Gmail</a>\
  ...
  execute: function email_execute({
    ...
    html = '<p><a href="'+ Utils.escapeHtml(URL) +'">'+
           Utils.escapeHtml(title) + '</a></p>\n' + html;
    title = "\u201c" + title + "\u201d";
    ...
    function last(xs) { return Array.slice(xs, -1)[0] }
  },
});
