...
function makeTestParser(lang, verbs, contextUtils) {
  ...
  return NLParser.makeParserForLanguage(lang, verbs, contextUtils,
                                         new TestSuggestionMemory());
}

var noun_arb_text = {
  rankLast: true,
  suggest: (text, html) => NounUtils.makeSugg(text, html, null, .3),
};
...
function testSortedBySuggestionMemory() {
  var verbList = [
    for (x of ["clock", "calendar", "couch", "conch",
               "crouch", "coelecanth", "crab"])
    {names: [(x + 1234567890).slice(0, 12)], id: x}
...
function testVerbUsesDefaultIfNoArgProvided() {
  var dog = new NounUtils.NounType("dog", ["poodle", "golden retreiver",
                                           "beagle", "bulldog", "husky"]);
  dog.default = () => NounUtils.makeSugg("husky")
  var verbList = [{
    names: ["wash"], DOType: dog, DOLabel: "dog",
  }, {
...
function testPartiallyParsedSentence() {
  ...
  var noun_type_baz = {
    ...
    default: function() { return NounUtils.makeSugg("super pants") },
  };
  ...
  }, {source: "from", instrument: "with", location: "near"});
...
function testEnParserRecursiveParse() {
  var {recursiveParse} =
    Cu.import("resource://ubiquity/modules/parser/locale_en.js", null);
  var {assertEquals} = this;
  var pp = olist => [for (o of olist) JSON.stringify(o)].sort() + "";
  var parse = (input, hasObj, preps) =>
