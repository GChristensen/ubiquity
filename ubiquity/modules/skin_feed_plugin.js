...
var SFP = Utils.extend(SkinFeedPlugin.prototype, {
  ...
  makeFeed(baseFeed, eventHub) {
    return SkinFeed(baseFeed, eventHub, this._msgService)
  },
  ...
  get skins() { return [
    for (feed of this._feedManager.getSubscribedFeeds())
    if (feed.type === "ubiquity-skin") feed
  ] },

  // === {{{ SkinFeedPlugin.customSkin }}} ===
  get customSkin() {
    return this._feedManager.getFeedForUrl(URL_CUSTOM)
  },

  // === {{{ SkinFeedPlugin.defaultSkin }}} ===
  get defaultSkin() {
    return this._feedManager.getFeedForUrl(URL_DEFAULT)
  },

  // === {{{ SkinFeedPlugin.currentSkin }}} ===
  get currentSkin() { return (
  ...
  toString() { return "[object SkinFeedPlugin]" },
});

// == SkinFeed ==

function SkinFeed(baseFeed, eventHub, msgService) { return Utils.extend({
  __proto__: baseFeed,
...
Utils.extend(SkinFeed.prototype, {
  ...
  get dataUri() {
    return Utils.uri("data:text/css;charset=utf-8,/*ubiquity-skin*/" +
  ...
  refresh() { return this },
  toString() { return "[object SkinFeed<" + this.uri.spec + ">]" },
});

UbiquityProtocol.set(URL_CUSTOM, () =>
  Utils.prefs.get(PREF_CUSTOM) ||
  Utils.getLocalUrl(URL_ROOT + "custom.css")
);
