...
var Utils = {
  toString() { return "[object UbiquityUtils]" },
  ...
  get currentChromeWindow() {
    return Utils.WindowMediator.getMostRecentWindow(Utils.appWindowType)
  },
  ...
  get currentTab() {
    return BrowserTab(Utils.currentChromeWindow.gBrowser.mCurrentTab)
  },

  // === {{{ Utils.currentTabs }}} ===
  // An array of tabs within the current chrome window.
  get currentTabs() { return gTabs.from(Utils.currentChromeWindow) },
...
;[
  ...
  function UnicodeConverter() { return (
  ...
  function appName() { return Services.appinfo.name },
...
function absolutifyUrlAttribute(element) {
  for (let attr of URL_ATTRS) if (attr in element) {
...
function openUrlInBrowser(urlString, postData) {
  ...
  if (!/^about:(?:blank|newtab)$/.test(browser.mCurrentBrowser.currentURI.spec) ||
...
function uniq(array, key, strict) {
  var f = key == null ? x => x : typeof key == "function" ? key : x => x[key]
  var args = [0, 1/0];
  if (strict) {
    let keys = [];
    for (let x of array) {
...
// === {{{ Utils.values(object) }}} ===
// Returns an array of all own, enumerable property values of {{{object}}}.

function values(obj) { return [for (k of keys(obj)) obj[k]] }

// === {{{ Utils.entries(object) }}} ===
// Returns an array of all own, enumerable property pairs
// ({{{[name, value]}}}) of {{{object}}}.

function entries(obj) { return [for (k of keys(obj)) [k, obj[k]]] }
...
function powerSet(arrayLike) {
  ...
  for (let i = 0, l = arrayLike.length; i < l; ++i) {
    ...
    for (let a of ps.concat()) ps.push(a.concat(next));
  }
  return ps;
}
...
function unescapeHtml(s) {
  return String(s).replace(unescapeHtml.re, unescapeHtml.fn)
}
unescapeHtml.re = /(?:&#?\w+;)+/g
unescapeHtml.fn = $ => Utils.UnescapeHTML.unescape($)
...
function BrowserTab(tabbrowser_tab) { return {
  __proto__: BrowserTab.prototype,
  raw: tabbrowser_tab,
} }
extend(BrowserTab.prototype, {
  get browser() { return this.raw.linkedBrowser },
  get tabbrowser() { return this.browser.getTabBrowser() },
  get uri() { return this.browser.currentURI },
  get title() { return this.browser.contentTitle },
  get window() { return this.browser.contentWindow },
  get document() { return this.browser.contentDocument },
  get icon() { return this.raw.image },
  get index() {
  ...
  get chromeWindow() {
    var {tabbrowser} = this;
    for (let win of Utils.chromeWindows)
      if (win.gBrowser === tabbrowser) return win;
    return null;
  },
  toJSON: function BT_toJSON() { return this.uri.spec },
  toString: function BT_toString() { return "[object BrowserTab]" },
  valueOf: function BT_valueOf() { return this.index },
...
var gTabs = Utils.tabs = {
  ...
  get length() {
    var num = 0;
    for (var win of Utils.chromeWindows) if ("gBrowser" in win)
  ...
  get: function tabs_get(name) {
    return name == null
    ? [...gTabs]
    : [for (t of gTabs) if (t.document.title === name ||
                            t.document.URL   === name) t]
  },
  ...
  reload: function tabs_reload(matcher) {
    for (let tab of gTabs.search(matcher)) tab.browser.reload();
    return this;
  },
...
gTabs[Symbol.iterator] = function* tabs_iter() {
  for (var win of Utils.chromeWindows) if ("gBrowser" in win) {
...
for (let n in gClipboard.flavors) {
  let name = n;
  gClipboard.__defineGetter__(name, () => gClipboard.get(name))
}
// === {{{ Utils.clipboard.text }}} ===
// Gets or sets the clipboard text.
gClipboard.__defineSetter__("text", txt => gClipboard.set({text: txt}))
...
Utils.history = {
  ...
  search: function history_search(query, callback) {
    var awesome = Cc["@mozilla.org/autocomplete/search;1?name=unifiedcomplete"]
    ...
    awesome.startSearch(query, "", null, {
      onSearchResult: function hs_onSearchResult(search, result) {
        switch (result.searchResult) {
          ...
          callback([for (i of Sequence(result.matchCount)) {
