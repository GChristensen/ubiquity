...
function FeedAggregator(feedManager, messageService, disabledCommands) {
  ...
  function FA_cmd_getDisabled() { return this.id in disabledCommands }
  ...
  self.onPageLoad = function FA_onPageLoad(document) {
    ...
    for (let pageLoadFuncList of pageLoadFuncLists)
      for (let pageLoadFunc of pageLoadFuncList)
        try {
  ...
  self.onUbiquityLoad = function FA_onUbiquityLoad(window) {
    ...
    for (let ubiquityLoadFuncList of ubiquityLoadFuncLists)
      for (let ubiquityLoadFunc of ubiquityLoadFuncList)
        try {
  ...
  self.refresh = function FA_refresh(anyway) {
    ...
    for (let feed of feeds) feed.refresh(anyway);
    ...
    for (let feed of feeds) {
      for (let cmd of Object.values(feed.commands || {})) {
  ...
  self.__defineGetter__("commandNames", () => commandNames)
  self.__defineGetter__("commandsByName", () => commandsByName);
  self.__defineGetter__("commandsByDomain", () => commandsByDomain);
...
function maybeRemindCommand(document, commandsByDomain) {
  ...
  var commands = [
    for (cmd of commandsByDomain[domain] || []) if (!cmd.disabled) cmd
  ];
...
function noNotificationSites() {
  return Utils.prefs.get(PREF_NNSITES, "").split("|")
}
