...
Gettext.prototype.get_lang_refs = () => []

var LocalizationUtils = {
  GETTEXT: new Gettext(),

  get loadedPo() { return loadedPo },

  isLocalizableLang(langCode) {
    return DefaultLanguageCodes.indexOf(langCode) < 0
  },

  isLocalizableFeed(feedUrl) { return (
  ...
  getLocalFeedKey(path, langCode) {
    return langCode + /\/\w+(?=\.\w+$)/.exec(path)
  },
  ...
  getFeedGlobals(feedUrl) {
    return feedGlobalsDict[feedUrl] || (feedGlobalsDict[feedUrl] = [])
  },

  registerFeedGlobal(feedUrl, key) {
    return LocalizationUtils.getFeedGlobals(feedUrl).push(key)
  },
  ...
  localizeCommand: function LU_localizeCommand(cmd, langCode) {
    ...
    for (let key of LocalizableProperties) if (cmd[key]) {
