...
var UbiquitySetup = {
  ...
  getBaseUri() { return this.baseUrl },
  ...
  get isResetScheduled() {
    return gPrefs.get(RESET_SCHEDULED_PREF, false)
  },
  set isResetScheduled(value) {
    return gPrefs.set(RESET_SCHEDULED_PREF, value)
  },
  ...
  get services() { return this.createServices() },
    ...
    if (!gServices) {
      ...
      var defaultFeedPlugin =
        new DefaultFeedPlugin(
          feedManager, msgService, gWebJsModule, "resource://ubiquity/");
  ...
  setupWindow: function setupWindow(window) {
    ...
    for (let id of ["appcontent", "sidebar"]) {
  ...
  get languageCode() {
    return gPrefs.getValue("extensions.ubiquity.language", "en")
  },

  set languageCode(code) {
    return gPrefs.setValue("extensions.ubiquity.language", code)
  },
