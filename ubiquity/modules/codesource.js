...
RemoteUriCodeSource.isValidUri =
  uri => Utils.uri(uri).scheme in VALID_SCHEMES_REMOTE
...
LocalUriCodeSource.isValidUri =
  (uri) => Utils.uri(uri).scheme in VALID_SCHEMES_LOCAL
...
LocalUriCodeSource.prototype = {
  ...
  getCode: function LUCS_getCode(throwNoError) {
    ...
    try {
      ...
      let code;
      if (this.uri.scheme === "ubiquity")
        code = UbiquityProtocol.get(this.uri.spec);
      else {
...
function XhtmlCodeSource(codeSource) {
  ...
  this.__defineGetter__("dom", () => dom)
  this.__defineGetter__("id", () => codeSource.id)
  this.__defineGetter__("codeSections", () => codeSections)
  ...
  this.getCode = function XCS_getCode() {
    ...
    if (/^\s*</.test(code)) {
      ...
      codeSections = [];
      finalCode = "";
      for (let info of parseCodeFromXml(code)) {
        let c = info.code;
        finalCode += c;
        codeSections.push({
          length: c.length,
