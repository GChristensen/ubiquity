...
function AnnotationService(connection) {
  ...
  self.getPagesWithAnnotation = function AS_getPagesWithAnnotation(name) {
    var pages = []
    for (var uri in ann) if (name in ann[uri]) pages.push(urls[uri])
    return pages
  }

  self.pageHasAnnotation = function AS_pageHasAnnotation(uri, name) {
    return uri.spec in ann && name in ann[uri.spec]
  }
  ...
  self.toJSONString = function AS_toJSONString() { return JSON.stringify(ann) }
