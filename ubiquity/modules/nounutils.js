...
function NounType(label, expected, defaults) {
  ...
  if (defaults) {
    ...
    this.default =
      Array.concat.apply(0, [for (d of defaults) this.suggest(d)]);
  }
}
...
NounType._fromArray = words => ({
  ...
  _list: [for (w of words) makeSugg(w)],
});
...
NounType._fromObject = function NT_Object(dict) {
  var list = Object.keys(dict).map(key => makeSugg(key, null, dict[key]));
  return {
    name: [for (s of list.slice(0, 2)) s.text] +
          (list.length > 2 ? ",..." : ""),
    _list: list,
  };
};
...
NounType._fromArray.suggest = NounType._fromObject.suggest =
  function NT_suggest(text) { return grepSuggs(text, this._list) }
...
function grepSuggs(input, suggs, key) {
  ...
  return [
    for (sugg of suggs) if ((match = re.exec(sugg[key])))
    (sugg.score = matchScore(match), sugg)
...
function mixNouns(label) {
  ...
  function mixer(key) { return function suggestMixed() {
    var val, suggsList = [
      for (noun of nouns) if ((val = noun[key]))
      typeof val == "function" ? val.apply(noun, arguments) : val
    ];
    return suggsList.concat.apply([], suggsList); // flatten
  } }
  return {
    label: gotLabel ? label : [for (n of nouns) n.label || "?"].join("|"),
    rankLast: nouns.some(n => n.rankLast),
    noExternalCalls: nouns.every(n => n.noExternalCalls),
    suggest: mixer("suggest"),
    default: nouns.some(n => "default" in n) && mixer("default"),
  };
}
